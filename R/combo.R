...
combo2exp <- function(str) {
  ...
  for(i in seq_along(mc_ops)){
    str <- str_replace_all(str, mc_ops[[i]][1], mc_ops[[i]][2])                   # translate logical operators
  }
# translate 0< operator; quote double character operators >=, <=, ==; mod operator %%;  and single character operators
  ere_sub <- list(c("(0<\\()", "<\\(0 "), c("([<>=]=)","'\\1'"), c("(%%)","'\\1'"), c("([-+*/<>!&|])(?=[^=0123456789])", "'\\1'"))
  for(i in seq_along(ere_sub)){
    str <- gsub(ere_sub[[i]][1], ere_sub[[i]][2], str, perl = TRUE)
  }
  gsub(" ", ", ", str_trim(str))
}
